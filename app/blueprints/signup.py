...
def signup():
    ...
    if request.method == "POST":
        ...
        if not name:
            ...
            # 3. Stamp the admin's email directly in their DB
            #    (must_change_password=0 because they chose this password themselves)
            db_path = os.path.join(Config.TENANTS_DIR, slug, "inventory.db")
            db = sqlite3.connect(db_path)
            db.execute(
                "UPDATE users SET email=?, must_change_password=0 WHERE username='admin'",
                [email])
            db.commit()
            db.close()

            # 4. Send welcome email (no-op if SMTP not configured)
            send_welcome_email(email, name, slug, password)
