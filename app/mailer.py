...
def send_welcome_email(to: str, name: str, slug: str, temp_password: str) -> bool:
    domain  = Config.APP_DOMAIN
    url     = f"https://{slug}.{domain}"
    subject = f"Welcome to CountDepot — your workspace is ready"
    html = f"""
<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:520px;margin:0 auto;padding:32px 24px;color:#0f172a">
  ...
  <div style="background:#f8f7f4;border-radius:10px;padding:18px 20px;margin-bottom:24px">
    ...
    <div style="display:flex;gap:32px;margin-top:14px">
      ...
      <div><div style="font-size:11px;color:#94a3b8;margin-bottom:2px">TEMP PASSWORD</div><strong style="font-family:monospace">{temp_password}</strong></div>
    </div>
  </div>
  <p style="font-size:13px;color:#64748b;margin-bottom:20px">You'll be asked to set a new password on first login, then choose your business type to set up the right categories.</p>
